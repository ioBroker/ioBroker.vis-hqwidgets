...
(function ($) {
                ...
                .bind('touchstart', function (e) {
                    ...
                    var event = $.Event(e.type, {
                        ...
                        originalEvent: {
                            touches: [
                                {
                                    pageX: e.originalEvent.touches[e.originalEvent.touches.length - 1].pageX,
                                    pageY: e.originalEvent.touches[e.originalEvent.touches.length - 1].pageY,
                                },
                            ],
                        },
                    });
    ...
    $.fn.popupShow = function ($div, options, callback) {
        ...
        return this.each(function () {
            ...
            $div.css({
                position: 'absolute',
                left: eLeft + ($this.width() - dw) / 2,
                top: eTop + ($this.height() - dh) / 2,
                'z-index': zindex,
            });
            setTimeout(function () {
            ...
            if (!dialog) {
                ...
                $dialog.dialog({
                    ...
                    open: function (event, ui) {
                        ...
                        $(this)
                            .parent()
                            .css('top', ($(window).height() - $(this).parent().height()) / 2);
...
if (vis.editMode) {
    $.extend(true, systemDictionary, {
        ...
        hoursLastAction: {
            en: 'Hide last action after(hrs)',
            de: 'Ausblenden letze Anderungszeit nach(Std)',
            ru: 'Скрыть последнее изменение(часов)',
        },
        ...
        descriptionLeftDisabled: {
            en: 'No description (left)',
            de: 'Keine Beschreibung (links)',
            ru: 'Без подписи (слева)',
        },
        ...
        'oid-slide-sensor-lowbat': {
            en: 'Slide sensor lowbat',
            de: 'FB-Sensor lowbat',
            ru: 'Сенсор на створке (lowbat)',
        },
        'oid-slide-handle': { en: 'Slide handle', de: 'Griff-Sensor', ru: 'Сенсор на ручке' },
        'oid-slide-handle-lowbat': {
            en: 'Slide handle lowbat',
            de: 'Griff-Sensor lowbat',
            ru: 'Сенсор на ручке (lowbat)',
        },
        door_type: { en: 'Door swing', de: 'Türtyp', ru: 'Тип двери' },
        noAnimate: { en: 'No animation', de: 'Keine Animation', ru: 'Не анимировать' },
        popupHorizontalPos: {
            en: 'Horizontal popup position',
            de: 'Horizontale PopUp Position',
            ru: 'Горизонтальное положение',
        },
        popupVerticalPos: {
            en: 'Vertical popup position',
            de: 'Vertikale PopUp Position',
            ru: 'Вертикальное положение',
        },
        ...
        infoLeftPaddingLeft: {
            en: 'Left padding (left)',
            de: 'Linker Abstand (Links)',
            ru: 'Отступ слева (левый текст)',
        },
        infoLeftPaddingRight: {
            en: 'Right padding (left',
            de: 'Rechter Abstand (Links)',
            ru: 'Отступ справа (левый текст)',
        },
        infoRightPaddingLeft: {
            en: 'Left padding (right)',
            de: 'Linker Abstand (Rechts)',
            ru: 'Отступ слева (правый текст)',
        },
        infoRightPaddingRight: {
            en: 'Right padding (right)',
            de: 'Rechter Abstand (Rechts)',
            ru: 'Отступ справа (правый текст)',
        },
...
vis.binds.hqwidgets = {
    ...
    button: {
                ...
                if (isHide) {
                    ...
                    if (reInit || !$c.length) {
                        ...
                        if (data.actual !== undefined && data.actual !== null) {
                            text +=
                                '<tr class="vis-hq-actual-style vis-hq-no-space"><td class="vis-hq-no-space"><span class="vis-hq-actual"></span>' +
                                (data.unit === undefined && data.unit === null ? '' : data.unit) +
                                '</tr>';
                        }
        ...
        draw: function ($div) {
            ...
            var radius =
                $div.css('borderTopLeftRadius') || vis.views[data.view].widgets[data.wid].style['border-radius'];
            ...
            if (!$div.find('.vis-hq-main').length) {
                ...
                if (data.offsetAuto) {
                    ...
                    text +=
                        '<table class="vis-hq-table vis-hq-no-space" style="position: absolute;top:' +
                        data.topOffset +
                        '%;left:' +
                        data.leftOffset +
                        '%"><tr class="vis-hq-no-space"><td class="vis-hq-no-space"><div class="vis-hq-icon" style="text-align: center;"></div></td>\n';
                }
            ...
            if (data.iconName || data.iconOn) {
                ...
                $div.find('.vis-hq-icon')
                    .html(
                        '<img class="vis-hq-icon-img" style="height: ' +
                            data.btIconWidth +
                            'px; width: auto;" src="' +
                            (img || '') +
                            '"/>',
            ...
            if (data.url /* && !vis.editMode*/) {
                $div.popupDialog({
                    content:
                        '<iframe src="' +
                        (data.url || '') +
                        '" style="width: 100%; height: calc(100% - 5px); border: 0"></iframe>',
        ...
        init: function (wid, view, data, style, wType) {
            ...
            data.styleNormal = data.usejQueryStyle
                ? 'ui-state-default'
    ...
    window: {
        ...
        hidePopup: function ($div) {
            ...
            if (data.noAnimate) {
                ...
                $big.animate(
                    { width: $div.width(), height: $div.height(), opacity: 0, top: 0, left: 0 },
                    500,
                    'swing',
                    function () {
                        $big.find('.hq-blind-big-slider').makeSlider('hide');
        ...
        openPopup: function ($div) {
            ...
            if (data.noAnimate) {
                ...
                $big.find('.hq-blind-big-slider').makeSlider('show', data.value);
                $big.animate(
                    { top: data.bigTop, left: data.bigLeft, width: data.bigWidth, height: data.bigHeight, opacity: 1 },
                    500,
                );
            }
        },
        draw: function ($div) {
            ...
            if (data.show_value) {
                if (data.infoRightPaddingLeft === undefined || data.infoRightPaddingLeft === null)
                    data.infoRightPaddingLeft = '15px';
                if (data.infoRightPaddingRight === undefined || data.infoRightPaddingRight === null)
                    data.infoRightPaddingRight = '15px';
                if (!data.infoRightPaddingRight.match(/px$|rem$|em$/))
                    data.infoRightPaddingRight = data.infoRightPaddingRight + 'px';
        ...
        init: function (wid, view, data, style) {
            ...
            data.hide_timeout =
                data.hide_timeout === 0 || data.hide_timeout === '0' ? 0 : parseInt(data.hide_timeout, 10) || 2000;
            ...
            data.digits = data.digits || data.digits === 0 ? parseInt(data.digits, 10) : null;
            data.noAnimate =
                data.noAnimate === 'true' || data.noAnimate === true || data.noAnimate === 1 || data.noAnimate === '1';
    ...
    door: {
            ...
            if (!notUpdateDoor) {
                if (value) {
                    if (data.noAnimate || isFirst) {
                        ...
                        $div.find('.vis-hq-door-handle').animate(
                            { left: data.door_type !== 'right' ? '60%' : '30%' },
                            500,
                        );
                    }
                } else {
                    if (data.noAnimate || isFirst) {
                        ...
                        $div.find('.vis-hq-door-handle').animate(
                            { left: data.door_type !== 'right' ? '85%' : '15%' },
                            500,
                        );
                    }
                }
        ...
        draw: function ($div) {
            ...
            if (!$div.find('.vis-hq-main').length) {
                ...
                text +=
                    '<table class="vis-hq-main vis-hq-door vis-hq-no-space" style="z-index: 1; position: absolute; top: 0; right: 0;">' +
        ...
        draw: function ($div, isInit) {
            ...
            if (!$img.length) {
                ...
                $big.css({
                    ...
                    width: data.popupRadius * 2,
                    height: data.popupRadius * 2,
                });
            ...
            if (
                ...
                vis.binds.hqwidgets.lock.isFalse(vis.states.attr(data.oid + '.val'), data.closeValue, data.openValue)
            ) {
    ...
    circle: {
        init: function (wid, view, data) {
            ...
            $scalaInput.attr(
                'data-linecap',
                settings.linecap === 'true' || settings.linecap === true ? 'round' : 'butt',
            );
            $scalaInput.show();
            ...
            if (settings.caption) {
                $scalaInput.after(
                    '<div style="position: absolute; left: 50%; top: 60%"><span style="position:relative; left: -50%" >' +
                        settings.caption +
                        '</span></div>',
                );
            }
    ...
    checkbox: {
        ...
        init: function (wid, view, data) {
            ...
            if (settings.oid && settings.oid !== 'nothing_selected') {
                ...
                $shineCheckbox.shineCheckbox(
                    'value',
                    vis.binds.hqwidgets.checkbox.isTrue(settings.staticValue, settings.max),
                );
            }
        },
    },
